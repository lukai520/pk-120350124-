...
public class DeleteUser extends JDialog implements WindowListener,ActionListener{
	...
	public void delUser()
	{
		...
		{
			JOptionPane.showMessageDialog(null, "请输入要删除的用户编号");
			return;
		}
		rs=au_db.getResult("select * from t_users where user_id='"+id_txf.getText().trim()+"'");
		if(id_txf.getText().trim().length()>20)
		{
			JOptionPane.showMessageDialog(null, "输入的用户编号过长!");
			return;
		}
		try {
			if(rs.next()==false)
			{
				JOptionPane.showMessageDialog(null, "不存在该用户"+id_txf.getText()+"");
				return;
			}
		...
		//如果该用户借的有书，则不能删除该用户
		rs=au_db.getResult("select * from t_users where user_id='"+id_txf.getText().trim()+"' and user_bookMount=0");
	
		try {
			if(rs.next()==false)
			{
				JOptionPane.showMessageDialog(null, "该用户还有书未还，不能删除!");
				return;
			}
		} catch (HeadlessException e) {
			e.printStackTrace();
		...
		{
			//先必须删除记录
			au_db.updateSql("delete from t_borrow where user_id='"+id_txf.getText().trim()+"'");
			au_db.updateSql("delete from t_record where user_id='"+id_txf.getText().trim()+"'");
			String delStringStr="delete from t_users where user_id='"+id_txf.getText().trim()+"'";
		...
		JOptionPane.showMessageDialog(null, "已成功删除"+id_txf.getText().trim()+"用户!");
		this.dispose();
